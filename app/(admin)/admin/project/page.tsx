...
export default function TableDemo() {
    ...
    const { data, error, isLoading } = useSWR<any, Error, string>(`${process.env.NEXT_PUBLIC_BDS_API_PROJECT}`, fetcher)
    const [projects, setprojects] = useState(data?.response?.data);
    const [search, setSearch] = useState('')
    const { data: searchQuery } = useSWR<any, Error, string>(`${process.env.NEXT_PUBLIC_BDS_API_PROJECT}?_search=${search}`, fetcher)
  

    const [quanhuyen, setDataquanhuyen] = useState<any>();
    useEffect(() => {
        fetch('https://vn-public-apis.fpo.vn/districts/getByProvince?provinceCode=01&limit=-1')
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    await removeProject(id)
                        .then(({ message }) => {
                            const updatedCategories = projects.filter((item: any) => item._id !== id);
        ...
        <div>
            <div className="flex justify-between my-10">
                ...
                <div className="flex justify-between my-10 ">
                    ...
                    <Button className="mr-5"><CiTrash /></Button>
                    <Button className='' variant="outline" ><Link href={'/admin/project/add'}><AiFillPlusCircle /></Link></Button>
                </div>
                    ...
                    {projects?.map((invoice: any, index: number) => (
                        <TableRow key={invoice?._id}>
                            ...
                            <TableCell className="text-right">
                                <Button className='mr-2' variant="outline"><Link href={`/admin/pe/update/`}>Cập nhật</Link></Button>
                                <Button className='' variant="outline" onClick={() => HandleRemovePrject(invoice._id)}>Xóa</Button>
                            </TableCell>
                        </TableRow>
                    ))}
                </TableBody>
